...
test('semantic bank validation: unknown bank key rejects hydration, locks form, and yields 0 POST calls', async () => {
  const doc = createMockAdminDoc();
  global.document = doc;
  global.Telegram = { WebApp: { initData: 'admin-init-data' } };
  global.GhostLinkV3 = {
  ...
  const mockFetch = async (url, options) => {
    ...
    // Server returns unknown bank 'vtb_crypto'
    return {
      ok: true,
      status: 200,
      json: async () => ({
  ...
  assert.equal(postCallsCount, 0, 'Must produce 0 POST calls when bank is unknown');
  assert.equal(toastMsg, 'Реквизиты не загружены. Повторите загрузку.');
});

test('semantic recipient validation: invalid recipient name format (<2 words or non-name) rejects hydration, locks form, and yields 0 POST calls', async () => {
  const doc = createMockAdminDoc();
  global.document = doc;
  global.Telegram = { WebApp: { initData: 'admin-init-data' } };
  global.GhostLinkV3 = {
  ...
  const mockFetch = async (url, options) => {
    ...
    // Server returns only 1 word for recipient
    return {
      ok: true,
      status: 200,
      json: async () => ({
  ...
  assert.equal(postCallsCount, 0, 'Must produce 0 POST calls when recipient format is invalid');
  assert.equal(toastMsg, 'Реквизиты не загружены. Повторите загрузку.');
});

test('semantic phone validation: invalid phone format (<10 digits) rejects hydration and locks form', async () => {
  const doc = createMockAdminDoc();
  global.document = doc;
  global.Telegram = { WebApp: { initData: 'admin-init-data' } };
  global.GhostLinkV3 = {
  ...
  const mockFetch = async (url, options) => {
    ...
    // Server returns invalid phone (<10 digits)
    return {
      ok: true,
      status: 200,
      json: async () => ({
  ...
  assert.equal(postCallsCount, 0, 'Must produce 0 POST calls when phone format is invalid');
  assert.equal(toastMsg, 'Реквизиты не загружены. Повторите загрузку.');
});
