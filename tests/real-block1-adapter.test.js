  ...
  const adapter = createRealBlock1Adapter({
    ...
    fetch: async (url) => {
      ...
      if (url.startsWith('https://panel.112prd.ru:2053')) {
        // Direct panel blocked or dropped
        throw new TypeError('Failed to fetch');
      }
      if (url.startsWith('https://api.112prd.ru:2053/api/miniapp/session')) {
        return response(200, { ok: true, session_token: 'api-session-token' });
      }
  ...
  const adapter = createRealBlock1Adapter({
    ...
    fetch: async (url) => {
      ...
      if (url.startsWith('https://panel.112prd.ru:2053')) {
        ...
        await new Promise((r) => setTimeout(r, 150));
        return response(200, { ok: true, session_token: 'slow-panel-token' });
      }
      if (url.startsWith('https://api.112prd.ru:2053/api/miniapp/session')) {
        // Fallback responds quickly in 10ms
        await new Promise((r) => setTimeout(r, 10));
        return response(200, { ok: true, session_token: 'fast-api-token' });
      }
      if (url.includes('/api/user')) {
        return response(200, {
